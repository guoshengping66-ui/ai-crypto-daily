...
def _render_ai_analysis_markdown_like(
    ...
    if result.signals:
        lines.extend(["**今日优先发布**", _format_list_content(result.signals), ""])

    if result.rss_insights:
        lines.extend(
            ["**信息核查**", _format_list_content(result.rss_insights), ""]
        )

    if result.outlook_strategy:
        lines.extend(
            ["**账号运营建议**", _format_list_content(result.outlook_strategy), ""]
        )
...
def render_ai_analysis_plain(result: AIAnalysisResult) -> str:
    ...
    if result.rss_insights:
        lines.extend(["[信息核查]", _format_list_content(result.rss_insights), ""])

    if result.outlook_strategy:
        lines.extend(["[账号运营建议]", _format_list_content(result.outlook_strategy), ""])
...
def render_ai_analysis_telegram(result: AIAnalysisResult) -> str:
    ...
    if result.rss_insights:
        lines.extend(["<b>信息核查</b>", _escape_html(_format_list_content(result.rss_insights)), ""])

    if result.outlook_strategy:
        lines.extend(["<b>账号运营建议</b>", _escape_html(_format_list_content(result.outlook_strategy)), ""])
...
def render_ai_analysis_html_rich(result: AIAnalysisResult) -> str:
    ...
    if result.signals:
        ...
        ai_html += f"""
                    <div class="ai-block">
                        <div class="ai-block-title">今日优先发布</div>
                        <div class="ai-block-content">{content_html}</div>
                    </div>"""
    ...
    if result.rss_insights:
        ...
        ai_html += f"""
                    <div class="ai-block">
                        <div class="ai-block-title">信息核查</div>
                        <div class="ai-block-content">{content_html}</div>
                    </div>"""
    ...
    if result.outlook_strategy:
        ...
        ai_html += f"""
                    <div class="ai-block">
                        <div class="ai-block-title">账号运营建议</div>
                        <div class="ai-block-content">{content_html}</div>
                    </div>"""
